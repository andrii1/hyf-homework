...
const getWeatherInCurrentLocation = (latitude, longitude) => {
  displayWeather.innerHTML = "";
...
buttonWeatherCurrentLocation.addEventListener("click", function () {
  navigator.geolocation.getCurrentPosition((pos) => {
    latitude = pos.coords.latitude;
    longitude = pos.coords.longitude;
    getWeatherInCurrentLocation(latitude, longitude);
  });
});
//Check if local storage exists
if (localStorage.getItem("location") != null) {
  const storageValue = localStorage.getItem("location");
  if (storageValue.includes("latitude")) {
    const coordinatesFromStorage = JSON.parse(localStorage.getItem("location"));
    latitude = coordinatesFromStorage.latitude;
    longitude = coordinatesFromStorage.longitude;
    getWeatherInCurrentLocation(latitude, longitude);
  } else {
    getCity(storageValue);
  }
}
