...
app.get("/last-name-count", async (req, res) => {
  const dbResult = await knex.raw(
    "SELECT * from users WHERE last_name='Doe' ORDER by first_name ASC"
  );
  const rows = dbResult[0];
  res.send({ rows });
});
...
app.get("/first-user", async (req, res) => {
  const dbResult = await knex.raw(
    "SELECT * from users WHERE first_name=b ORDER by created_at LIMIT 1"
  );
  const rows = dbResult[0];
  if (rows.length == 0) {res.send({"404"})} else {res.send({ rows });}
